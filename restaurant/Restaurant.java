...
//    root.add(logs,2,0);
    ...
    logs.setOnAction(e->{adminLogs(nested,current);});
    s.getStylesheets().add("/restaurant/styles.css");
    nested.setTitle("Admin Menu");
    nested.setScene(s);
    nested.show();
    }
private void adminLogs(Stage nested,Admin current){
...
nested.setScene(scene);
scene.getStylesheets().add("/restaurant/styles.css");  
nested.show();
}
...
private void menudetails(Stage nested, Admin current){
    ...
    bt2.getStyleClass().add("custom-button");
    bt4.getStyleClass().add("custom-button");
    bt5.getStyleClass().add("custom-button");
    GridPane root=new GridPane();
    root.add(bt1,0,0);
    root.add(bt2,1,0);
    root.add(bt4,3,0);
...
private void addmeal(Stage nested,Admin current){
    ...
    String it="";
    it+=system.Menu.getlist().get(i).getMenu_ID();
    items.add(it);
    }
    b1.setItems(items);
...
private void changemenu(Stage nested,Admin current){
    ...
    edit.setOnAction(e->{String getmenu=(String)b1.getValue();
                         ...
                         if(getcat.equals("Lunch")){
                             current.editMenu(id, MenuCategory.Lunch);
                         }
                         else if(getcat.equals("Dinner")){
                             current.editMenu(id, MenuCategory.Dinner);}
                         else if(getcat.equals("Breakfast")){
                         current.editMenu(id, MenuCategory.Breakfast);}
                         else if(getcat.equals("Dessert")){
                         current.editMenu(id, MenuCategory.Dessert); 
                         }
                         else if(getcat.equals("Beverages")){
                         current.editMenu(id, MenuCategory.Beverages);}
                         changemenu(nested,current);
                         });
    remove.setOnAction(e->{String getmenu=(String)b3.getValue();
                         int idd =Integer.parseInt(getmenu);
                         current.removeMenu(idd);
                         changemenu(nested,current);});
...
private void viewmenu(Stage nested,Admin current){
    ...
    b1.setOnAction(e->{
                        ...
                        for(int i=0;i<Menu.getlist().size();i++){
                            if(Menu.getlist().get(i).getMenu_ID()==id)
                                itemss.add(system.Menu.getlist().get(i).toString());
                        }
...
private void viewUsers(Stage nested,Admin current){
    ...
    receptionist.getStyleClass().add("custom-button");
    admin.getStyleClass().add("custom-button");
...
private void guestPreferencesMenu(Stage mainWindow,Receptionist current){
        ...
        back.getStyleClass().add("custom-button");
        btSelect.getStyleClass().add("custom-button");
        //Designing of layout
...
private void cancelReservationMenu(Stage mainWindow,Receptionist current){
        ...
        Button back=new Button("Back");
        btDelete.getStyleClass().add("custom-button");
        back.getStyleClass().add("custom-button");
        //Designing of layout
        HBox backBox=new HBox(back);
        backBox.setAlignment(Pos.CENTER_LEFT);
...
private void createReservationMenu(Stage mainWindow,Receptionist current){
        ...
        Button back=new Button("Back");
        btCrRes.getStyleClass().add("custom-button");
        btCancel.getStyleClass().add("custom-button");
        btAdd.getStyleClass().add("custom-button");
        back.getStyleClass().add("custom-button");
...
private void GuestScene(Stage nested,Guest current) {
        Button viewReservationButton = new Button("View Reservations");
        Button rateBookingButton = new Button("Rate Booking");
        viewReservationButton.getStyleClass().add("custom-button");
        rateBookingButton.getStyleClass().add("custom-button");
...
rateBookingScene(nested,current);}}); 
        ...
        HBox root=new HBox(10,viewReservationButton,rateBookingButton,back);
...
private void ViewReservationScene(Stage nested,Guest current) {
    ...
    ViewReservationLayout.setBackground(bg);
       bt1.setOnAction(e->{GuestScene(nested,current);});
...
private void rateBookingScene(Stage nested,Guest current) {
        ...
        submitRatingButton.getStyleClass().add("custom-button");
        Back.getStyleClass().add("custom-button");
        Exit.getStyleClass().add("custom-button");
        Back.setOnAction(e->{GuestScene(nested, current);});
        Exit.setOnAction(e->{
...
ad.show();
        ...
        HBox rateBookingLayout = new HBox(10);
