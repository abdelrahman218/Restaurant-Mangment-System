...
public class Meal implements Serializable {
  ...
  public ArrayList<Meal> getMealsByMenuId(int menuId) {
    ...
    /*public int OrdersInSpecificTime(Date StartDate, Date EndDate){
        int count=0;
        ArrayList<Meal> Meals=Meal.search(this);
        for(int i=0;i<Meals.size();i++)
            if(StartDate.before(Meals.get(i).getDate())||EndDate.after(Meals.get(i).getDate())
             ||StartDate.equals(Meals.get(i).getDate())||EndDate.equals(Meals.get(i).getDate())) 
                count++;
        return count;
    }*/
}